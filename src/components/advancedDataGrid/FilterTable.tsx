...
interface Props {
  ...
  setFilterData: Dispatch<
    ...
    >
  >;
  fieldsSelected: { first: string; second: string; third: string };
  setFieldsSelected: Dispatch<
    SetStateAction<{ first: string; second: string; third: string }>
  >;
}
...
const FilterTable = ({
  ...
  const handleFilter = (e) => {
    ...
    setFilterData(
      operation?.fn(data, selectedField, filterValueRef.current?.value)
    );
  };
  console.log(selectedField, new Date());
  return (
    <div className='flex items-center gap-2 w-1/2 m-4'>
      <Select
        ...
        options={columns
          .map(({ field }) => field)
          .filter((ele) => !Object.values(fieldsSelected).includes(ele))}
      />
      {type === "number" && (
        <Select
